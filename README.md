...
换一台电脑 clone 本项目后，普通运行命令不会自动下载或登录 MediaCrawler。需要先在仓库根目录执行初始化脚本；脚本会检测 `external/MediaCrawler` 是否存在，不存在时下载源码，并安装 Python 依赖和 Playwright 浏览器，然后启动用于登录的 CDP 浏览器并按平台做一次小流量登录/采集检查。
...
初始化时如果平台要求登录，请在弹出的浏览器中完成扫码或网页登录。初始化成功后会写入 `data/media_crawler_init/status.json`，正式研究只读取该状态并执行采集，不负责临时登录。
