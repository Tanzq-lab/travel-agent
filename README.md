...
默认请求模式是 `media_crawler`：优先通过 MediaCrawler 做真实公开资料采集。对小红书、B站、知乎、微博、贴吧等 UGC 平台，MediaCrawler 是主采集路径；公开网页或官方来源只能作为补充引用，不能替代平台采集语料。没有安装 MediaCrawler 或平台登录失败时，响应会在 `collection_errors` 中透明说明。也可以使用 `collection_mode=auto` 自动降级到 mock，但 auto/mock 结果不能作为正式旅游结论。
...
```powershell
python scripts\run_travel_research.py "我要 7 月去重庆，玩 3 天，带爸妈，不喜欢太累，怕热，预算中等。" --collection-mode media_crawler --limit 5
```

如果只想跑通 demo 或测试流程，可以显式使用 `auto` 或 `mock`，但这类结果不能作为正式旅游结论：

```powershell
python scripts\run_travel_research.py "我要去重庆" --collection-mode auto --limit 5
```
...
报告会明确写出“根据本次收集到的资料”。正式结论必须来自本次采集到的可追溯 evidence；如果 MediaCrawler 没有完成有效采集，系统只输出采集状态，不生成旅游结论、评分或推荐。
...
换一台电脑 clone 本项目后，普通运行命令不会自动下载、登录或操控浏览器。需要先在仓库根目录执行初始化脚本；脚本会检测 `external/MediaCrawler` 是否存在，不存在时下载源码，并安装 Python 依赖和 Playwright 浏览器，然后使用已运行的 CDP 浏览器按平台做一次小流量登录/采集检查。
...
默认情况下，初始化脚本不会启动浏览器。如果本机没有运行 `--remote-debugging-port=9222` 的 Chrome/Edge，脚本会停止并提示你手动启动。你也可以显式允许脚本启动一个专用浏览器 profile：

```powershell
.\scripts\initialize_media_crawler.ps1 -StartCdpBrowser
```

初始化时如果平台要求登录，请在浏览器中完成扫码或网页登录。初始化成功后会写入 `data/media_crawler_init/status.json`，正式研究只读取该状态并执行采集，不负责临时登录。

如果只想安装依赖、不做登录检查，可以单独运行：
...
MEDIA_CRAWLER_INIT_STATUS_PATH=data/media_crawler_init/status.json
```
...
如果 `MEDIA_CRAWLER_ROOT` 不存在、初始化状态缺失、请求平台未完成初始化，或初始化时启动的 CDP 浏览器不在运行，API 会返回清晰的 `collection_errors`，不会在正式研究阶段尝试扫码登录或补做初始化。
